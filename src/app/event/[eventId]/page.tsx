...
export default function CapturePhotoPage() {
  ...
  // phone's own native camera app (front camera) instead of our custom
  // in-app camera. Guests' photos taken through our old getUserMedia-based
  // capture came out visibly darker/noisier than the same shot taken with
  // the phone's own camera app (the live preview stream is tuned for smooth
  ...
  const confirmCrop = useCallback(() => {
    ...
    img.onload = () => {
      ...
      // Stash the untouched original (straight off the camera/upload, before
      // the blurred-background composite above) so the guest's actual photo
      // — not the effect applied to it — is what ends up in the "Source
      // Photos" folder later. Best-effort: sessionStorage is already tight
      // with the main image below, so a quota failure here just means no
      // source copy is saved, not a broken capture flow.
      try { sessionStorage.setItem('photobooth-original-image', rawImage); } catch { /* ignore */ }
        ...
        <AnimatePresence mode="wait">
          ...
          {mode === 'choose' && !capturedImage && (
            <motion.div key="choose" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              ...
              {/* `capture="user"` hands the shot off to the phone's own native
                  camera app, opened on the front/selfie camera (real quality/
                  HDR/flash, works on iPhone too) instead of our old in-app
                  getUserMedia camera. The guest can still flip to the rear
                  camera themselves using the native camera app's own button. */}
